...
namespace Game.Scripts.Inventory {
    public class InventoryPopup {
        ...
        private void CreateSlots(VisualElement container, int inventoryIndex, int rows, int columns) {
            var inventory = displayedInventories[inventoryIndex];
        ...
        private void OnSlotShiftClicked(Slot slot) {
            if (InventoryManager.instance.GetCurrentMode() != InventoryMode.DUAL) return;
        ...
        private void OnPointerUp(PointerUpEvent evt) {
            ...
            if (evt.button == 0) {
                ...
                ResetDragState();
            } else if (evt.button == 1 && dropTarget != null) {
        ...
        private Slot FindDropTarget(Vector2 position) {
            for (int invIndex = 0; invIndex < 2; invIndex++) {
                ...
                foreach (var slot in inventorySlots[invIndex]) {
                    ...
                    if (slot.worldBound.Contains(position)) return slot;
                }
            }
            return null;
        }
        ...
        private void CreateCraftingView() {
            var horizontal = UIBuilder.CreateContainer()
                .WithName("crafting-container")
                .WithSize(Length.Percent(100), Length.Percent(100))
                .WithFlexDirection(FlexDirection.Row)
                .WithJustifyContent(Justify.Center)
                .WithAlignItems(Align.Center)
            ...
            var inventoryPanel = CreateInventoryPanel("inventory-panel", 0);
            var craftingPanel = CreateCraftingPanel();

            horizontal.Add(inventoryPanel);
            horizontal.Add(craftingPanel);
            container.Add(horizontal);
        }

        private VisualElement CreateCraftingPanel() {
            var panel = UIBuilder.CreateContainer()
                .WithName("crafting-panel")
                .WithSize(300, 500)
                .WithBackgroundColor(panelColor)
                .WithBorders(3, new Color(0.25f, 0.2f, 0.15f), 12)
                .WithPaddings(15)
                .WithMargins(40, 0, 0, 0)
                .Build();
            ...
            var craftingGrid = UIBuilder.CreateContainer()
                .WithDisplay(UIDisplay.FLEX)
                .WithAlignItems(Align.Center)
                .WithJustifyContent(Justify.Center)
                .Build();

            panel.Add(craftingGrid);
            return panel;
        }
    }
}